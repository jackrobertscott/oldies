...
{
	const BRANDNAME = "bantanet";
	const WEBURL = "bantanet.com";
	const SUPPORTEMAIL = "support@bantanet.com";
	...
	*/
	...
	{
		$this->urlwtags = 'http://'.self::WEBURL.'/';
		$this->email_des['header'] = '<html>
										<body style="background-color: #efefef;margin: 0;padding: 0;font-family: arial;">
											...
											<div style="background-color: #ffffff;margin: 10px auto;padding: 10px;width: 560px;-webkit-border-radius: 3px;-moz-border-radius: 3px;border-radius: 3px;">
												<h1 style="text-align: center;margin: 20px 0 0 0;padding: 0;">'.self::BRANDNAME.'</h1>
												<h4 style="text-align: center;margin: 0;padding: 10px 0 0;">The Social University Timetable</h4>
												<br><hr>';
		$this->email_des['footer'] = 			'<br>
												<p>Sincerely<br><br>Team bantanet<br><br><a href="'.$this->urlwtags.'" style="text-decoration: none;">'.self::WEBURL.'</a></p>
												<hr>
												<p style="font-size: 10px;text-align: center;">'.self::BRANDNAME.'. 74 James Street, Northbridge, WA 6003</p>
												<p style="font-size: 10px;text-align: center;">You\'ve received this service announcement email to update you about changes within '.self::BRANDNAME.'.</p>
	...
	*/
	public function verifyemail($verCode, $direction)
	{
		...
		$phpemail->FromName   = self::BRANDNAME;
